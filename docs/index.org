...
**** TODO Konzept [0/4]                                          :Dominik:
SCHEDULED: <2021-05-09 Sun 12:00>

1. [ ] Funktionen des Prototypen definieren
2. [ ] Vorbild-App finden und Design an Funktionen anpassen
3. [ ] Feedback aus M2 einfließen lassen
4. [ ] Low-Fidelity-Prototype skizzieren
**** TODO App programmieren [0/3]
SCHEDULED: <2021-05-10 Mon 20:00>
1. [ ] App und HomeScreen erstellen
2. [ ] Restliche Screens aus LowFiPrototype implementieren
   z.B. RoomSelectionScreen, ScanScreen (Mock-Backend),
   ScanFeedbackScreen, ObjectSelectionScreen, DesignScreen,
   DesignFeedbackScreen
3. [ ] Kompilieren
...
*** Navigation
  ...
  1. 
     ...
     #+begin_quote
       Startbildschirm mit angenehmem Hintergrundbild und ein paar
       Auswahlbuttons (“Meine Räume”, “Neuen Raum scannen”, “Objekte”)
     #+end_quote
  ...
  2. 
	...
	#+begin_quote
          Es schaltet sich die Kamera ein, man ist im *RoomScanScreen*, es
          erscheint eine Info als Overlay
	#+end_quote
	...
	#+begin_quote
          Nach erfolgtem Scanvorgang wechselt die App in den
	...
	#+begin_quote
          Raummaße werden angezeigt. Buttons: OK (Raum speichern ->
          PopUp-Dialog für Eingabe des Namens, dann zurück zum Start), Neu
          scannen (zurück in den RoomScanScreen), Maße Manuell ändern
          (Funktion noch nicht implementieren, das würde zum
          RoomEditScreen führen), Abbruch (ohne Speichern zurück zum
          Start)
	#+end_quote
  ...
  3. 
	...
	#+begin_quote
          Liste zur Auswahl an Räumen erscheint (Name, Erstelldatum,
          Thumbnail)
	#+end_quote
	...
	#+begin_quote
          Bei Auswahl eines Raumes (alternative Optionen: Raum löschen,
          Raum duplizieren → Kopie wird erstellt, Raum bearbeiten → noch
          nicht implementieren → RoomEditScreen, siehe oben) gelangt man
          zum *RoomScreen*
	#+end_quote
	...
	#+begin_quote
          Im RoomScreen sieht man den Raum in (Fake-)3D, man kann sich
          herumdrehen, im Raum bewegen und man sieht mögliche bereits
          vorher hinzugefügte Möbelstücke
	#+end_quote
	...
	#+begin_quote
          Es gibt einen Button mit Plus-Symbol für das Hinzufügen neuer
          Objekte aus der Objekt-Bibliothek. Man gelangt hier in den
          *ObjectSelectScreen*
	#+end_quote
	...
	#+begin_quote
          Dieser Screen ähnelt dem RoomSelectScreen, man kann Objekte
          löschen, zum Hinzufügen auswählen und es gibt einen Plus-Button
          zum Hinzufügen neuer Objekte in die Bibliothek (Import und
          Scan). Die ObjectScan und ObjectImport-Funktionen müssen wir
	...
	#+begin_quote
          Hat man ein Objekt ausgewählt, gelangt man wieder zurück in den
          RoomScreen und das Objekt befindet sich mittig in der letzten
          Ansicht. Man kann es nun bearbeiten (löschen, verschieben) wie
          alle anderen Objekte auch.
	#+end_quote

     9. 

	#+begin_quote
          Zuletzt kann man den Raum speichern oder abbrechen. In beiden
  ...
  4. 
	...
	#+begin_quote
          Man gelangt in den *ObjectSelectScreen*, die Abfolge ist ident
          mit Pkt 3.f., nur dass man nach Auswahl eines Objekts in den
          *RoomSelectScreen* gelangt, um den Raum zu wählen, in den das
	...
	#+begin_quote
          Nach Auswahl des Raumes gelangt man in den *RoomScreen* und das
          neue Objekt ist in der mittig in der Ansicht platziert. Die
          weitere Abfolge ist ident mit Pkt. 3.h.
	#+end_quote
...
**** ObjectScanScreen (später)
     :PROPERTIES:
     :CUSTOM_ID: objectscanscreen-später
     :END:
...
**** RoomScanConfirmScreen
     :PROPERTIES:
     :CUSTOM_ID: roomscanconfirmscreen
     :END:
  ...

**** ObjectScanConfirmScreen (später)
     :PROPERTIES:
     :CUSTOM_ID: objectscanconfirmscreen-später
     :END:
  ...

**** RoomScreen
     :PROPERTIES:
     :CUSTOM_ID: roomscreen
     :END:
  ...

**** RoomSelectScreen
     :PROPERTIES:
     :CUSTOM_ID: roomselectscreen
     :END:
  ...

**** ObjectSelectScreen
     :PROPERTIES:
     :CUSTOM_ID: objectselectscreen
     :END:
  ...

*** Funktionen
    :PROPERTIES:
    :CUSTOM_ID: funktionen
    :END:
  (Kurze Auflistung und Beschreibung der notwendigen Funktionen)

  1. Auswahl Raum
	:PROPERTIES:
	:CUSTOM_ID: auswahl-raum
	:END:

  2. Auswahl Objekt
	:PROPERTIES:
	:CUSTOM_ID: auswahl-objekt
	:END:

  3. Scanfunktion
	:PROPERTIES:
	:CUSTOM_ID: scanfunktion
	:END:
...
*** Struktur Sourcecode
    :PROPERTIES:
    :CUSTOM_ID: struktur-sourcecode
    :END:

  - 

    #+begin_quote
      Sourcecode liegt auf
      [[https://github.com/hci-interior/app][_https://github.com/hci-interior/app_]]
    #+end_quote
  ...
  - 
    ...
    #+begin_quote
      Kleinere Components wie Buttons und Searchbars liegen in
      ./components
    #+end_quote
  ...
  - 
    ...
    #+begin_quote
      Idealerweise einheitlich entweder *.js oder *.jsx für Components und
      Screens (manche IDE Plugins wie linter und code formatter hängen von
      der Endung ab)
    #+end_quote
  ...
  - 
    ...
    #+begin_quote
      Änderungen lokal und auf einem eigenen Branch, z.B.
      “dominik-implement-objectscreen”, nicht direkt auf main-Branch
    #+end_quote

  - 

    #+begin_quote
      Wenn Änderung fertig und funktionsfähig wird in main-Branch
      integriert ( → pull request → merge)
    #+end_quote
...
*** Design
    ...
    :CUSTOM_ID: design
    :END:
  (Beispiele, Screenshots von Vorbild-Apps, Listen, Buttons, Farben usw.)
** App-Programmierung
*** Bonus-Features
...
*** Aufgabenstellung
 Implementieren Sie die App mit dem von Ihnen geplanten Spektrum an
 Aufgaben und Funktionen (high-fidelity Prototyp).

 Stellen Sie sicher dass Ihr Interface die Komplexität des
 zugrundeliegenden Problems realistisch widerspiegelt.

Anmerkung: technische Details des Backends dürfen prototypisch realisiert werden (high-fidelity Prototyp der App).
        
WICHTIG: Es soll sich hierbei um einen fertigen Prototypen bzw. eine fertige App handeln.

M4 ist nicht dazu da "noch zu implementieren was Sie in M3 nicht geschafft haben".

Ein unvollständiger Prototyp führt zu Punktabzug. Anmerkung: Ein nicht lauffähiger Prototyp gibt bis zu 75% Abzug der Gesamtpunkte.

Für formelle Dinge wie schlecht lesbaren oder strukturierten Code (sinnvolle Klassen- und Methodennamen, Code ist kommentiert, Einrückungen etc.) gibt es bis zu 10% Abzug der Gesamtpunkte.
